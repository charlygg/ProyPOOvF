...
public  class corteVentas extends Pantallas implements ActionListener{
	...
	private void paint() {
		...
		this.setTitle("Corte Ventas");
		this.setLocationRelativeTo(null);
		marco.setBackground(Color.black);
		
		lb1 = new JLabel("Corte de Ventas");
		...
		aa="# Corte   Fecha         Empleado   Total          Ventas";
		lb2 = new JLabel(aa);
	...
	private void crearConsulta() {
		...
		try {
			...
			res = st.executeQuery("select count(dtv_ven) as rentas from Detvent where dtv_fec='"+fecha+"'");
			while(res.next()){
	...
	private void total() {
		...
		try {
			st = cn.createStatement();
			res = st.executeQuery("select sum(dtv_sub) as suma from Detvent where dtv_fec='"+fecha+"'");
			while(res.next()){
	...
	private void guardar() {
		...
		try {
			...
			int r = st.executeUpdate("Insert into CorteVta (cov_id, cov_fec, cov_per, cov_tot, cov_vta) values (" 
					+nide+", '"
					+fecha+"', "
					+adm+", "
					+total+", "
	...
	private void autoid() {
		...
		try { 
			st = cn.createStatement();
			res = st.executeQuery("select max(cov_id) as max from CorteVta");
			if(res.next()){
	...
	public void crearArchivo(){
		try{
			archivosalida = new PrintWriter(new FileWriter(direccion+"corteVentas.doc"), true);
	...
	private void mostrar() {
		...
		try { 
			st = cn.createStatement();
			res = st.executeQuery("select * from CorteVta where cov_id="+txt.getText());
			if(res.next()){
				ta1.setText("");
				String elemento=res.getString("cov_id")+"            "+res.getString("cov_fec")+"   "+res.getString("cov_per")+"         "+res.getString("cov_tot")+"          "+res.getString("cov_vta");
				ta1.append(elemento+"\n");
			}
		} catch (SQLException e) {
			e.printStackTrace();
