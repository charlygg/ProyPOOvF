...
public  class verCorterenta extends Pantallas implements ListSelectionListener{
	
	public verCorterenta(){
	...
	private void paint() {
		...
		this.setTitle("Administrador | Menu | Corte de Renta");
		...
		lb1 = new JLabel("CORTES DE RENTAS");
	...
	private void crearTabla() {
		...
		try {
			...
			res = st.executeQuery("select * from CorteRen order by cor_id");
			while(res.next()){
				String elemento= res.getString("cor_id")
				+"   |   "+res.getString("cor_fec")
				+"      |   "+res.getString("cor_adm")
				+"           |   "+res.getString("cor_tot")
				+"           |   "+res.getString("cor_ren");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==bt2){
			if(modelolista.getSize()==0){
				colordialogo();
				JOptionPane.showOptionDialog(null,"Debe haber por lo menos 1 película rentada para hacer corte","Error",JOptionPane.DEFAULT_OPTION,JOptionPane.ERROR_MESSAGE,null,null,null);
			}else{
				corteRentas.actualizar(resultado);
				this.dispose();
			}
